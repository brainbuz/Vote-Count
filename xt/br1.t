...
note( "Precedencefile: $B1->{'PrecedenceFile'}");
note( "TieBreakerFallBackPrecedence: $B1->{'TieBreakerFallBackPrecedence'}");
...
# todo 'two level bottom runoff' => sub {
# my $B3 = Vote::Count->new(
#   BallotSet => read_ballots( 't/data/biggerset1.txt'),
#   PrecedenceFile => 't/data/biggerset1precedence.txt',
#   TieBreakerFallBackPrecedence => 1 );
# for ( qw( TOAD SOGGYCHIPS CHOCOANTS VOMIT)) { $B3->Defeat($_)}
# note( 'booorda runoof ******');
# p $B3->Borda()->RankTable();
# note( 'topcount runoof ******');
# p $B3->TopCount()->RankTable();
#  note( 'approval runoof ******');
# p $B3->Approval()->RankTable();
# note ( 'approval');
# p $B3->BottomRunOff('Approval') ;
# note ( 'approval- topcoutn');
# my $at = $B3->BottomRunOff('Approval', 'TopCount') ;
# note( '****************************');
# p $at;
# is ( $at->{'continuing'}, 'PISTACHIO', '');

# $at = $B3->BottomRunOff('Approval','precedence') ;
# is ( $at->{'eliminate'}, 'PISTACHIO', '');
