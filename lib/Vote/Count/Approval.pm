...
When counting Approval on Range Ballots it is possible and appropriate to set a threshold below which a choice is not considered to be supported by the voter, but only indicated to represent a preference to even lower or unranked choices. With a Range of 0-10 a cutoff of 5 would be recommended, and choices scored 4 or lower would not be counted for approval.

A good value is half of the maximum possible score, however, the default action must be to treat all ranked choices as approved. If cutoff isn't provided it defaults to 0 producing the desired default behaviour.

For Ranked Ballots the cutoff is ignored. If a cutoff is desired for Ranked Ballots a Neutral Preference Option should be included to indicate when subequent choices should not be considered Approved. This is not currently available in Vote::Count.
...
sub Approval ( $self, $active = undef, $cutoff=0 ) {
  ...
  if ( $BallotSet{'options'}{'rcv'} ) {
    ...
    _approval_range_do( $active, $BallotSet{'ballots'}, $BallotSet{'depth'}, $cutoff );
  }
}
