# Abstract Overview of Multi Member elections and Vote::Count support for them.
...
=head1 Overview of Multi-Member Methods
...
=head3 Approval, Cumulative Approval and X of Y
...
=head2 Generalized Description of STV
...
Seek the solution that leaves the least unused votes. Typically this type of method uses a calculation based on Surplus Transfer, and considers all possible combinations to find the set of winners which leaves the least unused vote. L<Vote::Count> does not currently attempt to implement an optimization method, but may consider them in the future.
