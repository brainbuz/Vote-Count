...
sub _ballotset( $ARGS ) {
  ...
  unless ( defined ( $ARGS->{'BallotSet'}{'choices'} )){
    croak "A Valid BallotSet or BallotFile was not provided " . $ARGS->{'BallotFile'} ."\n";}
}
...
sub _do_plurality ( $Election ) {
  ...
  $Election->logv( ' ', 'Initial Top Count (Plurality)', $Plurality->RankTable() );
...
sub _do_matrix( $Election) {
  ...
  $Election->logv(
    ...
    $matrix->MatrixTable(),
    "\nSmith Set: " .
    join( ', ', sort(keys $matrix->SmithSet()->%*) )
  );
